...
if __name__ == "__main__":
    ...
    if exists(f"../{subject_addr}.csv"):
        ...
        if file_size == 0:
            ...
            #     y="endBal",
            #     # color="assetSymbol",
            #     line_group="assetSymbol",
            # )

            fig = go.Figure()
            for index, symbol in enumerate(df["assetSymbol"].unique().tolist()):
                df_for_symbol = df[df["assetSymbol"] == symbol]
                fig.add_trace(
                    go.Bar(
                        name=symbol,
                        x=df_for_symbol["timestamp"],
                        y=df_for_symbol["endBal"],
